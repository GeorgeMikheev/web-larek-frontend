...
import { IBasket, ICard, IPopup, IOrder, IContactForm } from './types/interfaces';
...
events.on(allEvents.formErrorsOrderChange, (errors: Partial<IOrder>) => {
    const { payment, address } = errors;
    order.valid = !payment && !address;
    order.errors = Object.values({payment, address}).filter(i => !!i).join('; ');
});

events.on(allEvents.formErrorsContactChange, (errors: Partial<IContactForm>) => {
    const { email, phone } = errors;
    contactForm.valid = !email && !phone;
    contactForm.errors = Object.values({email, phone}).filter(i => !!i).join('; ');
});

events.on(allEvents.orderChange, (data: { field: keyof IOrder, value: string }) => {
    appData.setOrderField(data.field, data.value);
});

events.on(allEvents.contactFormChange, (data: { field: keyof IContactForm, value: string }) => {
